<template>
	<view id="test">
		...
		<!-- 内容 -->
		<view class="flex">
			<scroll-view scroll-y scroll-with-animation style="height:calc(100vh - 400rpx);flex: 1;">
				<!-- 左边 -->
			...
			<scroll-view scroll-y scroll-with-animation style="height:calc(100vh - 400rpx);flex: 3;" :scroll-into-view="'main-'+ mainCur"
			 @scroll="noScroll">
				<!-- 右边 -->
				<view class="flex-sub bg-red">
					<view v-for="(item, index) in rightNav" :key="index" :id="'main-'+ index">
						<view style="height: 260rpx; background-color: #023C41;">
							<view>
								<text>{{item.name}}</text>
							</view>
							<view class="flex justify-end text-center text-content">
								<view class="item bg-red" :class="rightListSum[index].delAnimation ? 'delRightShoppingCardAnimations' : 'delLeftShoppingCardAnimations'"
								 v-if="rightListSum[index].showDel" @click="delShoppingCard(index)">
									<text>-</text>
								</view>
								<view style="width: 60rpx;" v-if="rightListSum[index].showDel">
									<text>{{rightListSum[index].sum}}</text>
								</view>
								<view class="item bg-red" :data-index="index" @click="addShoppingCard">
									<text>+</text>
								</view>
							</view>
						</view>
			...
			createAnimation(ballX, ballY) {
				uni.getSystemInfo({
					success: async (e) => {
						...
						this.ballX = ballX -35; // 小球当前位置X
						this.ballY = ballY -35; // 小球当前位置Y
...
<style>
	...
	.delRightShoppingCardAnimations {
		animation: delRightShoppingCardAnimation .35s;
	}
